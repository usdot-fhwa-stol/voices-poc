        ...
        for crossing in all_sdsm_json_data:
            for sdsm_json in crossing["data"]:
                ...
                if sdsm_obj_index == None:
                    print(
                        "No VRU object found in SDSM: Crossing "
                        + str(crossing["crossing"])
                        + ", msgCnt "
                        + str(sdsm_json["msgCnt"])
                    )
                    continue

                vru_ref_pos = GeodeticToEcef(
                    sdsm_json["refPos"]["lat"], sdsm_json["refPos"]["long"], 245
                )
                ...
                world.debug.draw_string(
                    carla.Location(
                        x=local_vru_ref_pos_calc["x"],
                        y=local_vru_ref_pos_calc["y"],
                        z=draw_z_height,
                    ),
                ...
                print(
                    f"diff: x: {local_vru_ref_pos['x'] - local_vru_ref_pos_calc['x']} y: {local_vru_ref_pos['y'] - local_vru_ref_pos_calc['y']}"
                )

                world.debug.draw_string(
                    carla.Location(
                        x=local_vru_ref_pos["x"],
                        y=local_vru_ref_pos["y"],
                        z=draw_z_height,
                    ),
