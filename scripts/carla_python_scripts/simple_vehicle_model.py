...
def move_vehicle_with_point(world, vehicle, end_point, velocity):
    ...
    print(
        f"Simulation time: {world.get_snapshot().timestamp.elapsed_seconds:.2f} seconds"
    )
    print(
        "Vehicle location: x={}, y={}, z={}".format(location.x, location.y, location.z)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--x", type=float, default=0, help="x coordinate of the spawn point"
    )
    parser.add_argument(
        "--y", type=float, default=0, help="y coordinate of the spawn point"
    )
    parser.add_argument(
        "--z", type=float, default=0, help="z coordinate of the spawn point"
    )
    parser.add_argument(
        "--yaw", type=float, default=0, help="yaw orientation of the vehicle"
    )
    parser.add_argument(
        "--velocity", type=float, default=25, help="velocity of the vehicle in km/h"
    )
    parser.add_argument(
        "--stop",
        type=str,
        default="point",
        help="enter stop condition by duration or point",
    )
    parser.add_argument(
        "--duration", type=float, default=15, help="vehilce running duration"
    )
    parser.add_argument("--config", type=str, help="config file to import")
    parser.add_argument(
        "--point", nargs="+", default=[255, -130, 0], help="enter an end point"
    )
    parser.add_argument(
        "--svm_vehicle_name",
        default="NISSAN-SVM",
        help='Vehicle to be used for the simple vehicle model (default: "NISSAN-SVM"',
    )
    parser.add_argument(
        "--spawn_vehicle",
        action="store_true",
        help="adding this flag will spawn the vehicle as opposed to letting the carla adapter spawning the vehicle",
    )
    ...
    if args.config:
        ...
        for cfg_i, vehicle_cfg in enumerate(config["vehicles"]):
            ...
            vehicle.set_transform(
                carla.Transform(
                    carla.Location(x_pt, y_pt, z_pt), carla.Rotation(0, yaw, 0)
                )
            )
            ...
            scenario_results_header = pd.DataFrame(
                [],
                columns=[
                    "Scenario_#",
                    "initial_distance_to_entry_lane_stop_bar",
                    "speed (mph)",
                    "Dist_at_red (m)",
                ],
            )
            ...
            try:
                while reached_end == False:
                    ...
                    text_list.append(
                        font.render(
                            "|" + ("SCENARIO " + str(cfg_i + 1)).center(20) + "|",
                            True,
                            (255, 255, 255),
                        )
                    )
                    ...
                    # get scenario params
                    scenario_dist_to_stopbar = int(
                        round(
                            get_dist_3d_pythag(
                                vehicle_cfg["startPt"], vehicle_cfg["stopBar"]
                            ),
                            0,
                        )
                    )
                    ...
                    text_list.append(
                        font.render(
                            "Distance to Entry Lane Stop Bar = "
                            + str(scenario_dist_to_stopbar)
                            + "m ",
                            True,
                            (255, 255, 255),
                        )
                    ...
                    text_list.append(
                        font.render(
                            "Speed = " + str(vehicle_cfg["targetSpeedMPH"]) + " mph",
                            True,
                            (255, 255, 255),
                        )
                    )
                    ...
                    tl_actor = get_actor_by_location(
                        world, config["tl_location"], "traffic.traffic_light"
                    )
                    ...
                    text_list.append(
                        font.render(
                            "Desired Traffic Light State = "
                            + str(vehicle_cfg["targetTrafficLightState"]),
                            True,
                            (255, 255, 255),
                        )
                    )
                    text_list.append(
                        font.render(
                            "Current Traffic Light State = " + str(tl_state).ljust(6),
                            True,
                            (255, 255, 255),
                        )
                    )
                    ...
                    cur_vehicle_loc_array = np.array(
                        [cur_vehicle_loc.x, cur_vehicle_loc.y, cur_vehicle_loc.z]
                    )

                    cur_vehicle_vector = opp_stop_bar_point - cur_vehicle_loc_array

                    dist_from_int_exit = round(
                        get_dist_3d_vectors(start_vector, cur_vehicle_vector), 3
                    )

                    text_list.append(
                        font.render(
                            ("Current Distance to Exit Intersection = ").rjust(42)
                            + str(dist_from_int_exit).ljust(8)
                            + " m",
                            True,
                            (255, 255, 255),
                        )
                    ...
                    text_list.append(
                        font.render(
                            ("Distance to Exit Intersection at Red = ").rjust(42)
                            + str(dist_from_int_exit_at_red).ljust(8)
                            + " m",
                            True,
                            (255, 255, 255),
                        )
                    ...
                    if running:
                        text_list.append(
                            font.render("----- RUNNING -----", True, (255, 255, 255))
                        )
                    elif not started:
                        text_list.append(
                        ...
                        text_list.append(
                            font.render(
                                "----- WAITING FOR SIGNAL -----", True, (255, 255, 255)
                            )
                        )
                    ...
                    for i_t, text in enumerate(text_list):
                        text_center = text.get_rect(
                            center=(screen_width / 2, text_y_start + text_y_diff * i_t)
                        )
                    ...
                    if started == True:
                        if started == True and running == True:
                            ...
                            reached_end = move_vehicle_with_point(
                                world,
                                vehicle,
                                vehicle_cfg["endPt"],
                                vehicle_cfg["targetSpeedMPH"] / 2.23694,
                            )

                            if (
                                str(tl_state) == "Red"
                                and dist_from_int_exit_at_red == "########"
                            ):
                        ...
                        # need to make sure to start counting the start of the desired phase and no when script starts (in case of starting during desired phase)
                            ...
                            str(tl_state) != vehicle_cfg["targetTrafficLightState"]
                            and not fresh_desired_phase
                        ):
                        ...
                        # we need to do this because to protect against the default carla phase sequence which will flicker the traffic lights every so often
                            ...
                            str(tl_state) == vehicle_cfg["targetTrafficLightState"]
                            and fresh_desired_phase
                            and desired_phase_start == 0
                        ):
    ...
    # if x,y,z is passed, use those values for single run
        ...
        text = font.render(
            "Press SPACE to start vehicle movement", True, (255, 255, 255)
        )
        ...
        while True:
            ...
            if running:
                if args.stop == "time":
                    move_vehicle_with_duration(
                        world, vehicle, args.duration, args.velocity / 3.6
                    )
                elif args.stop == "point":
                    print("STOP POINT: " + str(args.point))
                    move_vehicle_with_point(
                        world, vehicle, args.point, args.velocity / 3.6
                    )
