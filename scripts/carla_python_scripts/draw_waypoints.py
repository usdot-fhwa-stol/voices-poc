...
def get_road_grade(start_point, end_point, mid_point):
    ...
    run = math.sqrt(
        (end_point[0].transform.location.x - start_point[0].transform.location.x) ** 2
    ...
    world.debug.draw_string(
        mid_point[0].transform.location,
        str(grade),
        draw_shadow=False,
...
def draw_waypoint_info(debug, w, lt=drawing_lifetime, x_offset=0, draw_data=False):
    ...
    if draw_data:
        debug.draw_string(
            w_loc + carla.Location(x=x_offset, z=0.5),
            f"lane: {w.lane_id}",
            False,
            yellow,
            lt,
        ...
        debug.draw_string(
            w_loc + carla.Location(x=x_offset, z=1.0),
            f"road: {w.road_id}",
            False,
            cyan,
            lt,
        ...
        debug.draw_string(
            w_loc + carla.Location(x=x_offset, z=1.5),
            f"lc: {w.lane_change}",
            False,
            red,
            lt,
        ...
        debug.draw_string(
            w_loc + carla.Location(x=x_offset, z=2),
            f"lt: {w.lane_type}",
            False,
            red,
            lt,
        ...
        debug.draw_string(
            w_loc + carla.Location(x=x_offset, y=0.5, z=2),
            f"x: {w.transform.location.x}",
            False,
            orange,
            lt,
        )
        debug.draw_string(
            w_loc + carla.Location(x=x_offset, y=1, z=2),
            f"y: {w.transform.location.y}",
            False,
            orange,
            lt,
        )
...
def draw_waypoints(world, map, waypoints, draw_arrows, veh_name):
    ...
    for i_sp in range(1, len(waypoints)):
        ...
        if i_sp == 1:
            ...
            general_route.append(
                f"0,{start_point.x},{start_point.y},{start_point.z},{start_point_geo.latitude},{start_point_geo.longitude},{start_point_geo.altitude}"
            )
            general_route.append(
                f"{i_sp},{end_point.x},{end_point.y},{end_point.z},{end_point_geo.latitude},{end_point_geo.longitude},{end_point_geo.altitude}"
            )
        else:
            general_route.append(
                f"{i_sp},{end_point.x},{end_point.y},{end_point.z},{end_point_geo.latitude},{end_point_geo.longitude},{end_point_geo.altitude}"
        ...
        if i_sp == len(waypoints) - 1:
            carma_route.append(
                f"{end_point_geo.longitude},{end_point_geo.latitude},0,{veh_name}_route"
            )
        else:
            carma_route.append(
                f"{end_point_geo.longitude},{end_point_geo.latitude},0,{veh_name}_route_waypoint_{i_sp}"
            )

        try:
            segment_waypoints = grp.trace_route(
    ...
    for waypoint, road_option in route_waypoints:
        ...
        if (
            segment_list[-1]["road_id"] == waypoint.road_id
            and segment_list[-1]["section_id"] == waypoint.section_id
            and segment_list[-1]["lane_id"] == waypoint.lane_id
        ):
            continue
        else:
    ...
    for segment in segment_list:
        ...
        draw_waypoint_union(
            debug, segment["starting_waypoint"], segment["ending_waypoint"], green
        )
        draw_waypoint_info(debug, segment["starting_waypoint"], draw_data=True)
        draw_waypoint_info(
            debug, segment["ending_waypoint"], x_offset=1, draw_data=True
        )
        ...
        while True:
            ...
            if len(next_wp) == 0:
                ...
                for fork_wp in next_wp:
                    print(
                        f"fork_wp.road_id: {fork_wp.road_id} final_segment_list road: {final_segment_list[i_seg + 1]['ending_waypoint'].road_id}"
                    )
                    if (
                        fork_wp.road_id
                        == final_segment_list[i_seg + 1]["ending_waypoint"].road_id
                    ):
    ...
    for i, waypoint in enumerate(final_waypoints):
        if draw_arrows:
            if i == 0:
                start_box_center = (
                    final_waypoints[i].transform.location + draw_arrow_z_offset
                )

                start_box = carla.BoundingBox(
                    start_box_center, carla.Vector3D(car_length, car_width, 0)
                )
                ...
                end_box = carla.BoundingBox(
                    end_box_center, carla.Vector3D(car_length, car_width, 0)
                )
                ...
                mid_box = carla.BoundingBox(
                    mid_box_center, carla.Vector3D(car_length / 2, car_width / 2, 0)
                )
        ...
        waypoint_data["x"].append(
            waypoint.transform.location.x
        )  # we swap x and y at the end for export to ltpENU
        ...
        waypoint_data["carla_bearing_yaw"].append(
            carla_bearing_yaw
        )  # this is transformed at the end
...
try:
    ...
    energy_campaign_left = {
        ...
        "waypoints": [
            ...
            carla.Location(
                x=-311.8514709472656, y=748.8836059570312, z=0.036188773810863495
            ),
            carla.Location(x=-88.226082, y=726.184509, z=-0.029539),
            carla.Location(
                x=305.2701110839844, y=722.4840698242188, z=0.036436766386032104
            ),
            carla.Location(
                x=603.3434448242188, y=830.186767578125, z=0.036185529083013535
            ),
        ],
    }
    ...
    if args.export:
        ...
        if not os.path.exists(folder_path):
            try:
                ...
                print(
                    "\tCreate directory waypoint_files within carla_python_scripts with read and write permissions for all users:"
                )
    ...
    if args.follow_vehicle:
        ...
        for i_v, veh_name in enumerate(spawn_data["veh_in_order"]):
            ...
            end_waypoint_index = (num_waypoints - 1) - (
                end_vehicle_wp_spacing * (len(spawn_data["veh_in_order"]) - 1 - i_v)
            )

            print(veh_name + " start_waypoint_index: " + str(start_waypoint_index))
            print(veh_name + " end_waypoint_index: " + str(end_waypoint_index))

            this_spawn = {"name": veh_name, "line_order": i_v, "waypoints": []}
        ...
        for spawn in new_spawns:
            ...
            waypoint_data = draw_waypoints(
                world, map, spawn["waypoints"], True, spawn["name"]
            )
            ...
            if args.export:
                ...
                df.to_csv(
                    "waypoint_files/" + spawn["name"] + "_breadcrumbs.csv", index=False
                )
