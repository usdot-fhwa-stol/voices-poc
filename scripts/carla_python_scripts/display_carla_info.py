        ...
        for index, vehicle in enumerate(vehicle_list, start=1):
            ...
            if map_string in map_height_dict:
                if (
                    ...
                    < map_height_dict[map_string]["bottom_line"]
                ):
                    continue
                elif (
                    vehicle.get_location().z > map_height_dict[map_string]["spawn_line"]
                ):
...
try:
    ...
    if map_string not in map_height_dict:
        print(
            "The height limits for map %s are unknown. Drawing all vehicle names as red..."
            % (map_string)
        )
