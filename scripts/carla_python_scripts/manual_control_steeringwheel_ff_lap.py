...
class World(object):
    def __init__(self, carla_world, hud, actor_filter):
        ...
        self._blueprint_list = self.world.get_blueprint_library().filter(
            self._actor_filter
        )
...
class DualControl(object):
    def __init__(self, world, start_in_autopilot):
        ...
        self._evtdev.write(ecodes.EV_FF, ecodes.FF_AUTOCENTER, int(65535 * 0.75))
        time.sleep(1)
        ...
        self._manualAutoToggle_idx = int(
            self._parser.get("G29 Racing Wheel", "manualAutoToggle")
        )
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                #    world.camera_manager.set_sensor(event.key - 1 - K_0)
                    ...
                    if world.player is not None:
                        transform = world.player.get_transform()
                        transform.location.x = 13.7
                        transform.location.y = -414.5
                        transform.location.z = 38
                        transform.rotation.roll = 0
                        transform.rotation.pitch = 0
                        transform.rotation.yaw = 7
                        world.player.set_transform(transform)
                        world.hud.lap_start_time = 0
                        world.hud.current_lap_time = 0

                elif event.key == K_2:
                    if world.player is not None:
                        transform = world.player.get_transform()
                        transform.location.x = 14.3
                        transform.location.y = -418.0
                        transform.location.z = 38
                        transform.rotation.roll = 0
                        transform.rotation.pitch = 0
                        transform.rotation.yaw = 7
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        if self.lap_start_time != 0:
            self.current_lap_time = currentTickTime - self.lap_start_time

        # if ((t.location.x >= 17.8) and (t.location.x <= 19) and (round(t.location.y,0) == -416) ):
        if (
            (t.location.x >= 17.8)
            and (t.location.x <= 19)
            and (t.location.y <= -412)
            and (t.location.y >= -419)
        ):
            if self.current_lap_time > 1:
                if self.lap_start_time > 0:
                    self.last_lap_time = self.current_lap_time
                    if self.best_lap_time == 0 or (
                        self.last_lap_time < self.best_lap_time
                    ):
                        self.best_lap_time = self.last_lap_time

            self.notification("      START LAP", seconds=2.0)
            self.lap_start_time = currentTickTime
        ...
        if isinstance(c, carla.VehicleControl):
            self._info_text += [
                ...
                "",
                "Lap Time:   % 6.2f s" % (self.current_lap_time),
                "Last Lap:   % 6.2f s" % (self.last_lap_time),
                "Best Lap:   % 6.2f s" % (self.best_lap_time),
                "",
            ]
...
class CameraManager(object):
    def __init__(self, parent_actor, hud):
        ...
        self._camera_transforms = [
            ...
            carla.Transform(
                carla.Location(x=0.150, y=-0.30, z=1.25), carla.Rotation(yaw=-55)
            ),
            carla.Transform(
                carla.Location(x=0.150, y=-0.30, z=1.25), carla.Rotation(yaw=55)
            ),
            carla.Transform(
                carla.Location(x=0.150, y=0, z=1.25), carla.Rotation(yaw=180)
            ),
...
def game_loop(args):
    ...
    try:
        ...
        display = pygame.display.set_mode((args.width, args.height), SCREEN_MODE)
        ...
        stop_bar_north = carla.Location(x=19, y=-419.4, z=38.15)
        stop_bar_south = carla.Location(x=17.8, y=-412.6, z=38.15)

        debug = world.world.debug

        debug.draw_line(
            stop_bar_north, stop_bar_south, 0.3, carla.Color(r=255, g=0, b=0), int(0)
        )
