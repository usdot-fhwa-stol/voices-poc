        ...
        while self.player is None:
            if args.x and args.y and args.z:
                ...
                spawn_point = carla.Transform(
                    ...
                    carla.Rotation(yaw=args.yaw, roll=args.roll, pitch=args.pitch),
                )
            else:
...
class KeyboardControl(object):
        ...
        if not self._autopilot_enabled:
            if isinstance(self._control, carla.VehicleControl):
                self._parse_vehicle_keys(
                    pygame.key.get_pressed(), clock.get_time(), world, args
                )
    ...
    def _parse_vehicle_keys(self, keys, milliseconds, world, args):
        ...
        if keys[K_UP] or keys[K_w]:
            ...
            self._control.throttle += smoothing * (
                target_throttle - self._control.throttle
            )

            # Clamp final throttle just in case
            self._control.throttle = max(
                min(self._control.throttle, max_throttle), min_throttle
            )
            ...
            decel_rate = 0.03
            self._control.throttle = max(
                self._control.throttle - decel_rate, min_throttle
            )
