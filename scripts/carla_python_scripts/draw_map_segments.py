...
def get_reference_point_in_carla_coordinates(intersection_map_json):
    ...
    referencePointLatLon = carla.GeoLocation(
        referenceLat, referenceLon, referenceElevation
    )
...
def ingest_points_from_map_data(intersection_map_json):
    intersectionID = intersection_map_json["mapData"]["intersectionGeometry"][
        "referencePoint"
    ]["intersectionID"]
    referenceLat = float(
        intersection_map_json["mapData"]["intersectionGeometry"]["referencePoint"][
    ...
    print(
        "Reference Point: %f deg %f deg %f m"
        % (referenceLat, referenceLon, referenceElevation)
    )

    approachList = intersection_map_json["mapData"]["intersectionGeometry"]["laneList"][
        "approach"
    ]
    segmentList = list(
        map(
            lambda approach: (
                list(
                    map(
                        lambda lane: list(
                            map(
                                lambda segmentPair: Segment(
                                    "approachID-%s-laneID-%s"
                                    % (approach["approachID"], lane["laneID"]),
                                    float(segmentPair[0]["nodeLat"]),
                                    float(segmentPair[0]["nodeLong"]),
                                    float(segmentPair[0]["nodeElev"]),
                                    float(segmentPair[1]["nodeLat"]),
                                    float(segmentPair[1]["nodeLong"]),
                                    float(segmentPair[1]["nodeElev"]),
                                    CarlaColor.GREEN
                                    if approach["approachType"] == "Ingress"
                                    else CarlaColor.ORANGE,
                                ),
                                list(
                                    zip(lane["laneNodes"][:-1], lane["laneNodes"][1:])
                                ),  # segmentPairs
                            )
                        ),
                ...
                if (
                    approach["approachType"] == "Ingress"
                    or approach["approachType"] == "Egress"
                )
                else []
            ),
            approachList,
...
def draw_line_segment(segment, translation):
    ...
    strt_point = (
        carla.Location(x=segment.startx, y=segment.starty, z=0) * scalingfactor
    ) + translation
    stop_point = (
        carla.Location(x=segment.endx, y=segment.endy, z=0) * scalingfactor
    ) + translation
...
def generate_points():
    ...
    A1a = A2a + np.array(
        [
            -(intersection_length / 2.0) + 1.8,
            (intersection_length / 2.0) - 0.88 + (lane_width / 2.0),
            0,
        ]
    )
    A3a = A2a + np.array(
        [
            (intersection_length / 2.0) + 1.35,
            (intersection_length / 2.0) + (lane_width / 2.0) - 0.5,
            0,
        ]
    )
    ...
    return [
        Segment(
            "A2a_B4a", A2a[0], A2a[1], A2a[2], B4a[0], B4a[1], B4a[2], CarlaColor.GRAY
        ),
        Segment(
            "A2b_B4b", A2b[0], A2b[1], A2b[2], B4b[0], B4b[1], B4b[2], CarlaColor.GRAY
        ),
        Segment(
            "A2a_A4a", A2a[0], A2a[1], A2a[2], A4a[0], A4a[1], A4a[2], CarlaColor.GRAY
        ),
        Segment(
            "A2b_A4b", A2b[0], A2b[1], A2b[2], A4b[0], A4b[1], A4b[2], CarlaColor.GRAY
        ),
        Segment(
            "A1a_A3a", A1a[0], A1a[1], A1a[2], A3a[0], A3a[1], A3a[2], CarlaColor.GRAY
        ),
        Segment(
            "A1b_A3b", A1b[0], A1b[1], A1b[2], A3b[0], A3b[1], A3b[2], CarlaColor.GRAY
        ),
        Segment(
            "A4a_C2a", A4a[0], A4a[1], A4a[2], C2a[0], C2a[1], C2a[2], CarlaColor.GRAY
        ),
        Segment(
            "A4b_C2b", A4b[0], A4b[1], A4b[2], C2b[0], C2b[1], C2b[2], CarlaColor.GRAY
        ),
        Segment(
            "A1a_D3a", A1a[0], A1a[1], A1a[2], D3a[0], D3a[1], D3a[2], CarlaColor.GRAY
        ),
        Segment(
            "A1b_D3b", A1b[0], A1b[1], A1b[2], D3b[0], D3b[1], D3b[2], CarlaColor.GRAY
        ),
        Segment(
            "A3a_E1a", A3a[0], A3a[1], A3a[2], E1a[0], E1a[1], E1a[2], CarlaColor.GRAY
        ),
        Segment(
            "A3b_E1b", A3b[0], A3b[1], A3b[2], E1b[0], E1b[1], E1b[2], CarlaColor.GRAY
        ),
    ]
...
try:
    ...
    if selection == 1:
        ...
        intersection_map_segment_list = ingest_points_from_map_data(
            intersection_map_json
        )
