...
def check_for_vehicles(vehicle_rolename_actor_map, vehicle_list):
    ...
    for vehicle_actor in vehicle_actor_list:
        ...
        if vehicle_actor.attributes["role_name"] in vehicle_list:
            ...
            vehicle_rolename_actor_map[vehicle_actor.attributes["role_name"]][
                "vehicle_actor"
            ] = vehicle_actor
...
def get_next_prev_waypoint_grade(waypoint):
    ...
    if len(next_waypoint_list) > 0 and len(prev_waypoint_list) > 0:
        next_prev_waypoint_grade = get_road_grade(
            prev_waypoint_list[0].transform.location,
            next_waypoint_list[0].transform.location,
...
try:
    ...
    for vehicle_rolename in vehicle_list:
        ...
        vehicle_rolename_actor_map[vehicle_rolename]["filename"] = (
            str(timestr) + "_" + str(vehicle_rolename) + "_eco_data.csv"
        )

        # write headers
        df = pd.DataFrame(eco_data_base)
        df.to_csv(
            eco_data_folder_path
            + "/"
            + vehicle_rolename_actor_map[vehicle_rolename]["filename"],
            header=True,
            index=False,
    ...
    while True:
        if not found_all_veh:
            vehicle_rolename_actor_map = check_for_vehicles(
                vehicle_rolename_actor_map, vehicle_list
            )
            ...
            for vehicle_rolename in vehicle_rolename_actor_map:
                if (
                    vehicle_rolename_actor_map[vehicle_rolename]["vehicle_actor"]
                    == None
                ):
                    found_all_veh = False
                    break
        ...
        for vehicle_rolename in vehicle_rolename_actor_map:
            ...
            this_veh_actor = vehicle_rolename_actor_map[vehicle_rolename][
                "vehicle_actor"
            ]
