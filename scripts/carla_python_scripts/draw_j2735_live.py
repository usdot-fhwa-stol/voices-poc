...
def convert_lat_long_to_universal(lat, lon, alt=0):
    ...
    tcm_refpoint_x, tcm_refpoint_y, tcm_refpoint_z = transformer.transform(
        lat, lon, alt
    )

    print(
        f"tcm_refpoint_x {tcm_refpoint_x}, tcm_refpoint_y {tcm_refpoint_y}, tcm_refpoint_z {tcm_refpoint_z}"
    )
...
def convert_universal_to_carla(x_u, y_u, z_u=0):
    ...
    # Perform the transformation
    (
        tcm_to_carla_refpoint_x,
        tcm_to_carla_refpoint_y,
        tcm_to_carla_refpoint_z,
    ) = tcm_to_carla_transformer.transform(x_u, y_u, z_u)

    return {
        "x": tcm_to_carla_refpoint_x,
        "y": -1 * tcm_to_carla_refpoint_y,
        "z": tcm_to_carla_refpoint_z,
    }
...
def draw_tcm(tcm_json):
    ...
    msg_ref_pos_carla = convert_universal_to_carla(
        msg_ref_pos["x"], msg_ref_pos["y"], msg_ref_pos["z"]
    )
    ...
    for tcm_node in msg_content["geometry"]["nodes"]:
        ...
        node_carla = convert_universal_to_carla(
            node_universal["x"], node_universal["y"], node_universal["z"]
        )
    ...
    for segment_start, segment_end in zip(tcm_nodes, tcm_nodes[1:]):
        draw_box_from_two_points_and_width(
            segment_start["x"],
            segment_start["y"],
            segment_start["z"],
            segment_end["x"],
            segment_end["y"],
            segment_end["z"],
            msg_content["geometry"]["refwidth"] / 100,
        )


def draw_box_from_two_points_and_width(
    start_point_x,
    start_point_y,
    start_point_z,
    end_point_x,
    ...
    tcm_segment_bearing = calculate_bearing(
    ...
    tcm_segment_length = math.sqrt(
        (start_point_x - end_point_x) ** 2 + (start_point_y - end_point_y) ** 2
    )

    print(f"tcm_segment_length: {tcm_segment_length}")

    box_center = carla.Location(
        x=tcm_segment_midpoint["x"], y=tcm_segment_midpoint["y"], z=draw_z_height
    )

    msg_box = carla.BoundingBox(
        box_center, carla.Vector3D(tcm_segment_length / 2, tcm_segment_width / 2, 0)
    )
...
def draw_tcr(tcr_json):
    ...
    for tcr_node in offsets_including_0_0:
        ...
        node_carla = convert_universal_to_carla(
            node_universal["x"], node_universal["y"], 0
        )
...
def draw_box_from_opposite_corners(zero_corner, opposite_corner):
    ...
    box_center = carla.Location(
        x=box_midpoint["x"], y=box_midpoint["y"], z=draw_z_height
    )

    msg_box = carla.BoundingBox(
        box_center, carla.Vector3D(tcr_segment_length / 2, tcr_segment_width / 2, 0)
    )
