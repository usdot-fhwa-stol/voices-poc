...
from run_synchronization import (
    SimulationSynchronization,  # pylint: disable=wrong-import-position
)
from sumo_integration.carla_simulation import (
    CarlaSimulation,  # pylint: disable=wrong-import-position
)
from sumo_integration.sumo_simulation import (
    SumoSimulation,  # pylint: disable=wrong-import-position
)
...
def write_sumocfg_xml(
    cfg_file, net_file, vtypes_file, viewsettings_file, additional_traci_clients=0
):
    """
    ...
    try:
        ...
        if args.safe:
            blueprints = [
                x
                for x in blueprints
                if vtypes[x]["vClass"] not in ("motorcycle", "bicycle")
            ]
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "--sumo-host", default=None, help="IP of the sumo host server (default: None)"
    )
    argparser.add_argument(
        "--sumo-port",
        default=None,
