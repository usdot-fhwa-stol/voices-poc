    ...
    for edge in sumo_net.getEdges():
        for lane in edge.getLanes():
            if lane.getParam("origId") is None:
                raise RuntimeError(
                    'Sumo lane {} does not have "origId" parameter. Make sure that the --output.original-names parameter is active when running netconvert.'.format(
                        lane.getID()
                    )
                )
            ...
            if len(lane.getParam("origId").split()) > 1:
                logging.warning(
                    "[Building topology] Sumo net contains joined opendrive roads."
                )
            ...
            for odr_id in lane.getParam("origId").split():
                ...
                odr2sumo_ids[(odr_road_id, int(odr_lane_id))].add(
                    (edge.getID(), lane.getIndex())
                )
    ...
    for from_edge in sumo_net.getEdges():
        for to_edge in sumo_net.getEdges():
            ...
            for connection in connections:
                ...
                topology[(from_edge_id, from_lane_index)].add(
                    (to_edge_id, to_lane_index)
                )
...
class SumoTrafficLight(object):
    ...
    Connection = collections.namedtuple(
        "Connection", "tlid from_road to_road from_lane to_lane link_index"
    )
    ...
    def generate_default_program(tl):
        ...
        for road in set(incoming_roads):
            ...
            tl.add_phase(
                SumoTrafficLight.DEFAULT_DURATION_GREEN_PHASE, "".join(phase_green)
            )
            tl.add_phase(
                SumoTrafficLight.DEFAULT_DURATION_YELLOW_PHASE, "".join(phase_yellow)
            )
            tl.add_phase(
                SumoTrafficLight.DEFAULT_DURATION_RED_PHASE, "".join(phase_red)
            )

    def add_phase(
        self, duration, state, min_dur=-1, max_dur=-1, next_phase=None, name=""
    ):
        """
        Adds a new phase.
        """
    ...
    def add_landmark(
        self, landmark_id, tlid, from_road, to_road, from_lane, to_lane, link_index=-1
    ):
        """
        Adds a new landmark.
        ...
        def is_same_connection(c1, c2):
            return (
                c1.from_road == c2.from_road
                and c1.to_road == c2.to_road
                and c1.from_lane == c2.from_lane
                and c1.to_lane == c2.to_lane
            )

        connection = SumoTrafficLight.Connection(
    ...
    def to_xml(self):
        ...
        for phase in self.phases:
            ET.SubElement(
                xml_tag,
                "phase",
                {"state": phase.state, "duration": str(phase.duration)},
            )
        ...
        for from_lane, to_lane in landmark.get_lane_validities():
            ...
            for lane_id in range(from_lane, to_lane + 1):
                ...
                if wp.is_junction and sumo_topology.is_junction(road_id, lane_id):
                    ...
                    if guess_tls:
                        for from_edge, from_lane in sumo_topology.get_incoming(
                            road_id, lane_id
                        ):
                            successors = sumo_topology.get_successors(
                                from_edge, from_lane
                            )
                        ...
                        connections = sumo_topology.get_path_connectivity(
                            road_id, lane_id
                        )
                ...
                elif not wp.is_junction and not sumo_topology.is_junction(
                    road_id, lane_id
                ):
                    print(f"\t\tWP is not junction")
                    from_edge, from_lane = sumo_topology.get_sumo_id(
                        road_id, lane_id, landmark.s
                    )

                    for to_edge, to_lane in sumo_topology.get_successors(
                        from_edge, from_lane
                    ):
                        ...
                        tl.add_landmark(
                            landmark.id, tl.id, from_edge, to_edge, from_lane, to_lane
                        )
